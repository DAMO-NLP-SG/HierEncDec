...
from generation_utils import GenerationMixin # chenhui: using customized generation_utils
...
def _load_state_dict_into_model(model_to_load, state_dict, start_prefix):
    ...
    # chenhui: extend positional embedding to allow longer source for BART
    try:
        print("using transformers version 4.24.0")
        dim_modified_keys = []
        dim_modified_values = []
        init_state_dict = model_to_load.state_dict()
        ...
        if len(init_state_dict_emb_pos) == len(state_dict_emb_pos):
            for (init_state_key, state_key) in zip(init_state_dict_emb_pos, state_dict_emb_pos):
                ...
                if preload_size < init_size:
                    print(f"extending [{state_key}] using model initialized [{init_state_key}] from {preload_size} to {init_size}")
                    dim_modified_keys.append(state_key)
                    init_val = init_state_dict[init_state_key] 
                    repeat = init_size // preload_size
                    remainder = init_size % preload_size
                    new_val = torch.cat([state_dict[state_key].clone()]*repeat+[state_dict[state_key].clone()[:remainder,:]],0)
                    dim_modified_values.append(new_val)
                else:
                    print(f"using embed_position size: {preload_size}")
...
def _load_state_dict_into_meta_model(
    ...
    # chenhui: extend positional embedding to allow longer source for BART
    try:
        logger.info("using transformers version 4.24.0")
        dim_modified_keys = []
        dim_modified_values = []
        ...
        if len(init_state_dict_emb_pos) == len(state_dict_emb_pos):
            for (init_state_key, state_key) in zip(init_state_dict_emb_pos, state_dict_emb_pos):
                ...
                preload_size = state_dict[state_key].size(0)
                if preload_size < init_size:
                    logger.info(f"extending [{state_key}] using model initialized [{init_state_key}] from {preload_size} to {init_size}")
                    dim_modified_keys.append(state_key)
                    init_val = init_state_dict[init_state_key] 
                    repeat = init_size // preload_size
                    remainder = init_size % preload_size
                    ...
                    logger.info(f"using embed_position size: {preload_size}")

        for key, val in zip(dim_modified_keys, dim_modified_values):
            state_dict[key] = val
    except: 
        logger.info("error during counting embedding length, not extending length")
